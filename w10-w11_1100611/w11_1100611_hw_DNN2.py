...
#tfboard lordir
log_dir = "C:\\Users\\Girl\\Desktop\\python\\python_course\\students\\w10-w12\\w10_1100611\\logs\\fit\\" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir, histogram_freq=1)    # type: ignore
...
model.add(tf.keras.layers.Dense(units=512, kernel_initializer='normal',activation=activation_function, name='layers_dense_2')) #type: ignore
# Add output layer
model.add(tf.keras.layers.Dense(units=10,  kernel_initializer='normal',activation='softmax', name='layers_dense_3'))    # type: ignore
